<template>
  <div class="intro-section">
    <div class="content-wrapper">
      <h1>
        <span class="highlight">How does bus network structure shape system costs and emissions of fleet electrification? Evidence from 224 Chinese cities</span>
        <br>
      ...
      <div class="links-container">
        ...
        <a href="https://github.com/huajide/China-eBus" class="link">Code</a>
        <span class="divider">•</span>
        <a href="./map/china-city/" target="_blank" class="link">Map</a>
      </div>
...
onMounted(() => {
  const subtitleElement = subtitle.value;
...
.intro-section {
  ...
  text-align: center;
  position: relative;
  background: linear-gradient(to bottom, rgba(8, 95, 135, 0.214), transparent);
  overflow: hidden;
}
...
.intro-section::before {
  ...
  background:
    /* Circuit board lines */
    linear-gradient(to right, rgba(87, 103, 224, 0.282) 1px, transparent 1px) 0 0 / 40px 40px,
    linear-gradient(to bottom, rgba(144, 167, 241, 0.608) 1px, transparent 1px) 0 0 / 40px 40px;
  opacity: 0.3;
  z-index: -1;
...
.intro-section .highlight {
  background: linear-gradient(90deg, var(--vp-c-green-2) 0%, var(--vp-c-indigo-1) 100%);
...
.decorative-line {
  width: 80%;
  height: 2px;
  background: linear-gradient(to right, var(--vp-c-green-2), var(--vp-c-indigo-1), transparent);
  border: none;
  margin: 1.5rem auto;
}
